...
const getEnd = (c) => (c.end_date || c.end ? new Date(c.end_date || c.end) : null);
...
const buildMoodDistribution = async ({ userId, user_id }) => {
  ...
  for (const note of notes) {
    const mood = typeof note.mood === 'string' ? note.mood.trim().toLowerCase() : null;
...
const getInsightsForUser = async (userIdOrErd) => {
  ...
  let insights = await Insight.findOne({ userId: uid }).lean();
  if (!insights) insights = await recomputeForUser(uid);
  return insights;
};
...
const recomputeForUser = async (userIdOrErd) => {
  ...
  const cyclesAsc = [...cycles].sort((a, b) => new Date(a.start_date || a.start) - new Date(b.start_date || b.start));
  ...
  const cyclesDesc = [...cyclesAsc].sort((a, b) => new Date(b.start_date || b.start) - new Date(a.start_date || a.start));
  const moodDistributionLast30d = await buildMoodDistribution({ user_id: uid });
  ...
  const updated = await Insight.findOneAndUpdate({ userId: uid }, { $set: payload }, { upsert: true, new: true, setDefaultsOnInsert: true }).lean();
  return updated;
};
...
const predictNextPeriods = async (userIdOrErd, count = 3) => {
  ...
  const cyclesDesc = await Cycle.find({ $or: [{ user_id: uid }, { userId: uid }] }).lean();
  if (!cyclesDesc.length) return { nextStarts: [] };

  const cyclesAsc = [...cyclesDesc].sort((a, b) => new Date(a.start_date || a.start) - new Date(b.start_date || b.start));
  const averageCycleLength = calculateAverageCycleLength(cyclesAsc);
  if (!averageCycleLength || averageCycleLength <= 0) return { nextStarts: [] };

  const limit = Math.max(1, Math.min(Number.parseInt(count, 10) || 3, 6));
  const nextStarts = [];
  let reference = new Date(cyclesDesc.sort((a, b) => new Date(b.start_date || b.start) - new Date(a.start_date || a.start))[0].start_date || cyclesDesc[0].start);

  for (let i = 0; i < limit; i += 1) {
    reference = new Date(reference.getTime() + averageCycleLength * MS_PER_DAY);
